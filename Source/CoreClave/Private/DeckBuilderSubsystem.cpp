...
bool UDeckBuilderSubsystem::LoadFromSlot()
{
	...
	if (UDeckBuilderSaveGame* SaveGame = Cast<UDeckBuilderSaveGame>(UGameplayStatics::LoadGameFromSlot(SaveSlotName.ToString(), SaveUserIndex)))
	{
		...
		if (bMigratedSaveGame)
		{
			...
			{
				// 저장에 실패해도 현재 실행의 메모리 데이터는 이미 보정된 상태로 계속 사용한다.
...
bool UDeckBuilderSubsystem::SaveToSlot()
{
	...
	// 저장 실패 시 원래 상태로 되돌리기 위해 기존 저장 여부 임시 보관
	const bool bPreviousSaveState = WorkingDeck.bHasBeenSaved;

	// 사용자가 Save버튼을 누른것이므로 true
	// 이 부분을 Commit전에 설정해야 true로 설정한 값도 DeckCollection에 복사가 진행된다.
	...
	if (!bSaved)
	{
		// 저장에 실패했다면 메모리에서도 저장된 덱으로 표시되지 않도록 설정
...
bool UDeckBuilderSubsystem::MigrateLoadedSaveGame(UDeckBuilderSaveGame& SaveGame)
{
	...
	// 버전 1에서 bHasBeenSaved 필드가 추가됐다.
	// 구버전 SaveGame의 DeckCollection에는 편집 후 저장된 덱만 들어 있으므로, 카드가 존재하는 슬롯을 저장된 덱으로 복원한다.
	...
	// 마이그레이션할 데이터가 없어도 구버전 번호를 올려 같은 검사를 반복하지 않게 한다.
	...
	{
		// 더 최신 버전의 저장 파일을 구버전 게임이 덮어쓰지 않도록 버전을 낮추지 않는다.
